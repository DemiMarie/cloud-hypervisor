...
use crate::vm_config::{
    ...
    DeviceConfig, DiskConfig, FsConfig, NetConfig, PmemConfig, UserDeviceConfig, UserVhostConfig,
...
const USER_VHOST_DEVICE_NAME_PREFIX: &str = "_user_vhost";
...
pub enum DeviceManagerError {
    ...
    /// Cannot create virtio-user-vhost device
    #[error("Cannot create virtio-user-vhost device")]
    CreateVirtioUserVhost(#[source] virtio_devices::vhost_user::Error),
    ...
    /// Virtio-user-vhost device was created without a socket.
    #[error("Virtio-user-vhost device was created without a socket")]
    NoVirtioUserVhostSock,
...
impl DeviceManager {
    ...
    fn make_virtio_devices(&mut self) -> DeviceManagerResult<()> {
        ...
        // Add virtio-user-vhost if required
        self.make_virtio_user_vhost_devices()?;
    ...
    fn make_virtio_user_vhost_device(
        &mut self,
        user_vhost_cfg: &mut UserVhostConfig,
    ) -> DeviceManagerResult<MetaVirtioDevice> {
        let id = if let Some(id) = &user_vhost_cfg.id {
            id.clone()
        } else {
            let id = self.next_device_name(USER_VHOST_DEVICE_NAME_PREFIX)?;
            user_vhost_cfg.id = Some(id.clone());
            id
        };

        info!("Creating virtio-user-vhost device: {user_vhost_cfg:?}");

        let mut node = device_node!(id);

        if let Some(user_vhost_socket) = user_vhost_cfg.socket.to_str() {
            let virtio_user_vhost_device = Arc::new(Mutex::new(
                virtio_devices::vhost_user::UserVhost::new(
                    id.clone(),
                    user_vhost_socket,
                    user_vhost_cfg.queue_sizes.clone(),
                    user_vhost_cfg.device_type,
                    None,
                    self.seccomp_action.clone(),
                    self.exit_evt
                        .try_clone()
                ...
                .map_err(DeviceManagerError::CreateVirtioUserVhost)?,
            ));

            // Update the device tree with the migratable device.
            node.migratable =
                Some(Arc::clone(&virtio_user_vhost_device) as Arc<Mutex<dyn Migratable>>);
            self.device_tree.lock().unwrap().insert(id.clone(), node);

            Ok(MetaVirtioDevice {
                virtio_device: Arc::clone(&virtio_user_vhost_device)
                    as Arc<Mutex<dyn virtio_devices::VirtioDevice>>,
                iommu: false,
                id,
                pci_segment: user_vhost_cfg.pci_segment,
                dma_handler: None,
            })
        } else {
            Err(DeviceManagerError::NoVirtioUserVhostSock)
        }
    }

    fn make_virtio_user_vhost_devices(&mut self) -> DeviceManagerResult<()> {
        let mut vhost_user_devices = self.config.lock().unwrap().user_vhost.clone();
        if let Some(vhost_user_list_cfg) = &mut vhost_user_devices {
            for vhost_user_cfg in vhost_user_list_cfg.iter_mut() {
                let device = self.make_virtio_user_vhost_device(vhost_user_cfg)?;
                self.virtio_devices.push(device);
            }
        }
        self.config.lock().unwrap().user_vhost = vhost_user_devices;
