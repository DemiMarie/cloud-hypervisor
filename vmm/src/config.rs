...
pub enum Error {
    ...
    ParseFsSockMissing,
    /// UserVhost socket is missing
    #[error("Error parsing --user-vhost: socket missing")]
    ParseUserVhostSockMissing,
    /// UserVhost number of queues is missing
    #[error("Error parsing --user-vhost: number of queues missing")]
    ParseUserVhostNumResponseQueuesMissing,
    /// UserVhost virtio ID is missing
    #[error("Error parsing --user-vhost: virtio ID missing")]
    ParseUserVhostVirtioIdMissing,
    /// UserVhost available features is missing
    #[error("Error parsing --user-vhost: available features missing")]
    ParseUserVhostAvailFeaturesMissing,
    #[error("Error parsing --user-vhost: queue size {0} is {1}, but limit is 65535")]
    ParseUserVhostQueueSizeTooLarge(usize, u64),
    /// UserVhost default queues is missing
    #[error("Error parsing --user-vhost: default queues missing")]
    ParseUserVhostDefaultQueuesMissing,
    /// UserVhost queue size missing
    #[error("Error parsing --user-vhost: queue size missing")]
    ...
    #[error("Error parsing --user-vhost")]
...
impl UserVhostConfig {
    pub const SYNTAX: &'static str = "virtio-user-vhost parameters \
...
impl VmConfig {
    ...
    pub fn validate(&mut self) -> ValidationResult<BTreeSet<String>> {
        ...
        if let Some(user_vhosts) = &self.user_vhost {
            if !user_vhosts.is_empty() && !self.backed_by_shared_memory() {
                return Err(ValidationError::VhostUserRequiresSharedMemory);
            }
            for user_vhost in user_vhosts {
                user_vhost.validate(self)?;

                Self::validate_identifier(&mut id_list, &user_vhost.id)?;
            }
        }
    ...
    pub fn parse(vm_params: VmParams) -> Result<Self> {
        ...
        let mut user_vhost: Option<Vec<UserVhostConfig>> = None;
        if let Some(user_vhost_list) = &vm_params.user_vhost {
            let mut user_vhost_config_list = Vec::new();
            for item in user_vhost_list.iter() {
                user_vhost_config_list.push(UserVhostConfig::parse(item)?);
            }
            user_vhost = Some(user_vhost_config_list);
        }
...
impl Clone for VmConfig {
    fn clone(&self) -> Self {
        VmConfig {
            ...
            fs: self.fs.clone(),
            user_vhost: self.user_vhost.clone(),
...
mod unit_tests {
    ...
    fn make_vhost_user_config(
        ...
        assert!(!id.contains(",[]\n\r\0\""));
        let config = UserVhostConfig::parse(&format!(
...
id=\"{id}\",pci_segment={pci_segment},queue_sizes={queue_sizes}"
    ...
    fn test_parse_vhost_user() -> Result<()> {
        ...
        UserVhostConfig::parse("").unwrap_err();
        UserVhostConfig::parse("virtio_id=1").unwrap_err();
        UserVhostConfig::parse("queue_size=1").unwrap_err();
        UserVhostConfig::parse("socket=/tmp/sock").unwrap_err();
        UserVhostConfig::parse("id=1").unwrap_err();
