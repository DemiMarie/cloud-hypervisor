...
impl UserVhost {
    /// Create a new virtio-user-vhost device.
        ...
        let (avail_features, acked_features, acked_protocol_features, vu_num_queues, paused) =
            if let Some(state) = state {
                ...
                if num_queues > backend_num_queues {
                    error!(
                        "user-vhost requested too many queues ({num_queues}) \
...
impl VirtioDevice for UserVhost {
    ...
    fn read_config(&self, offset: u64, mut data: &mut [u8]) {
        if let Some(vu) = &self.vu_common.vu {
            ...
            {
                panic!("Failed getting user-vhost configuration: {e}");
            }
        } else {
            panic!("Cannot get configuration space with no vu")
        }
    }
    ...
    fn write_config(&mut self, offset: u64, data: &[u8]) {
        if let Some(vu) = &self.vu_common.vu {
            ...
            {
                panic!("Failed setting user-vhost configuration: {e:?}");
            }
        } else {
            panic!("Cannot get configuration space with no vu")
        }
...
impl Snapshottable for UserVhost {
    fn id(&self) -> String {
        self.id.clone()
    }
...
impl Transportable for UserVhost {}

impl Migratable for UserVhost {
