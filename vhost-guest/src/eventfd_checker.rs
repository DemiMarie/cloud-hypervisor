...
pub(super) fn check_is_stream_socket(fd: OwnedFd) -> io::Result<UnixStream> {
    unsafe fn check_int_getsockopt(
        ...
        let size = mem::size_of_val(&expected) as libc::socklen_t;
...
#[derive(Debug)]
pub(crate) struct EventfdChecker {
    root: ProcRoot,
    st_dev: u64,
    is_anon_inode: bool,
}
...
/// [`EventFd`].
fn borrow_eventfd<'a>(eventfd: &'a EventFd) -> BorrowedFd<'a> {
...
unsafe fn os_err<T, U: FnOnce(*mut T) -> c_int>(cb: U) -> Result<T, io::Error> {
...
fn fstatfs(fd: BorrowedFd) -> io::Result<libc::statfs64> {
...
fn openat2(
    ...
    let r = {
        ...
        let mut how: MaybeUninit<libc::open_how> = unsafe { mem::zeroed() };
...
impl EventfdChecker {
    ...
    #[allow(clippy::useless_conversion)]
    fn from_proc_root(root: ProcRoot) -> io::Result<Self> {
    ...
    fn check_is_eventfd(&self, fd: BorrowedFd) -> io::Result<bool> {
    ...
    pub(super) fn convert_to_eventfd(&self, fd: OwnedFd) -> Result<EventFd, (OwnedFd, Error)> {
