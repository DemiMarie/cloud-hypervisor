...
pub struct FrontendRequestQueuePair<T: Allocator, U: VM> {
    queue_pair: queue_pair::VirtioVhostGuestQueuePair,
...
impl<T: Allocator, U: VM> FrontendRequestQueuePairInternals<T, U> {
    fn set_mem_table(&mut self, buf: &mut [u8], fd: &mut [Option<OwnedFd>]) -> Result<(), Error> {
        const _: () = assert!(
            u64::MAX as usize as u64 == u64::MAX && usize::MAX as u64 as usize == usize::MAX,
        ...
        const _: () = assert!(align_of::<VhostUserMemory>() == 1);
        ...
        // SAFETY: Bounds checked above, VhostUserMemory has alignment 1,
        ...
        let ctx: &mut [VhostUserMemoryRegion] = unsafe {
            slice::from_raw_parts_mut(
                buf.as_mut_ptr().add(size_of::<VhostUserMemory>()).cast(),
                num_regions,
            )
        };
        ...
        // Reset the mapping
        self.mapping.reset()?;
        for (region, file) in ctx.iter_mut().zip(fd.iter_mut()) {
            region.user_addr = self
                .mapping
    ...
    fn process_f2b_requests(
        ...
        match req {
            ...
            FrontendReq::SET_DEVICE_STATE_FD | FrontendReq::CHECK_DEVICE_STATE => {
    ...
    fn set_log_base(&mut self, buf: &mut [u8], fd: &mut [Option<OwnedFd>]) -> Result<(), Error> {
        ...
        let file = Self::get_single_file(fd)?;
        let mut raw_region: VhostUserLog = read_bytevalued(buf).ok_or(Error::InvalidMessage)?;
        let mut region = VhostUserMemoryRegion {
            guest_phys_addr: 0,
            memory_size: raw_region.mmap_size,
            user_addr: 0,
            mmap_offset: raw_region.mmap_offset,
        };
        self.mapping.check_region(&region)?;
        region.guest_phys_addr = u64::MAX;
        region.user_addr = u64::MAX;
        raw_region.mmap_offset = self
            .mapping
            .map_region_without_alignment_checks(region, file.as_fd())?
            .try_into()
            .unwrap();
        self.seen_log_mapping = true;
        buf.copy_from_slice(raw_region.as_slice());
        Ok(())
    }
    ...
    fn add_mem_reg(&mut self, buf: &mut [u8], fd: &mut [Option<OwnedFd>]) -> Result<(), Error> {
        ...
        let offset: u64 = self
            .mapping
            .map_region(*region, file.as_fd())?
            .try_into()
            .unwrap();
        // SAFETY: buffer is checked to be 40 bytes above.
        // FIXME: use safe code
        unsafe {
            buf.as_mut_ptr()
                .cast::<u64>()
    ...
    fn vring_kick(
        ...
        self.vm.register_vring_kick(eventfd, index).map_err(|e| {
            error!("Vring kick register error: {e}");
            Error::BackendInternalError
        })
    }
...
impl<T: Allocator, U: VM> FrontendRequestQueuePair<T, U> {
    ...
    pub fn set_fd(&mut self, fd: UnixStream) -> bool {
        self.queue_pair.set_socket(fd).is_ok()
    }
